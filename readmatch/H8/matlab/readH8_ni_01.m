...
% CALIPSO H8时间匹配
% 得到H8经纬度序列
Lat_H8 = round(100*(60-(0:0.02:120))); % H8纬度序列
Lon_H8 = round(100*(80+(0:0.02:120))); % H8经度序列
% 获取对应H8网格经纬度
Lat_H8_1=round(Lat_CAL/0.02)*2; % CALIPSO点最近的H8点经纬度，得到一个目标H8经纬度
Lon_H8_1=round(Lon_CAL/0.02)*2;
Lon_H8_1=max(8000,Lon_H8_1);    % 不能超过H8经纬度范围
...
% 读取h8t为格式datetime
DT_CAL = DT_CAL - HoursH8pixel; % 去除像元成像时间，'HHmmss'
DT_CAL.Format = 'HHmmss';
DT_CAL1 = char(DT_CAL);
caldt = str2double(string(DT_CAL1(:,3:6))); % 得到mmss
H8T = cal2h8dt1(caldt); % 转换最近H8的mm00，H8为每十分钟一次观测
H8t = [DT_CAL1(:,1:2),H8T]; % HH + mm00
