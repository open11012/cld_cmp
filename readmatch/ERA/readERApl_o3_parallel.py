...
# pressure levels
files = ['ozone_mass_mixing_ratio_2017']
varnames = ['o3']
pressure_level = [
    '1', '2', '3',
    '5', '7', '10',
    ...
    for i in range(len(files)):
        ...
        for ii in tqdm(range(len(data)), desc=num_mon + ' ' + varname):
            ...
            if pd.to_datetime(dt).month == month:
                df.loc[n] = var.loc[dt, :, lat, lon].values  # type: ignore
            else:
                num_mon = '%02d' % pd.to_datetime(dt).month
                filename = files[i] + num_mon + '.nc'
                pathname = filepath + filename
                if os.path.isfile(pathname):
                    dataset1 = xr.open_dataset(pathname)
                    var1 = dataset1[varname]
                    df.loc[n] = var1.loc[dt, :, lat, lon].values  # type: ignore
                else:
                    df.loc[n] = np.nan
            n = n+1
        data_out = pd.concat([data, df], axis=1)
        filename = '/home/fuhy/data/SWC/data/ERA5/plevels/2017/ERApl_' + varname + '2017' + num_mon + '.csv' # 这里保存名称中的月份可能因为最后一个数据而变为下一个月
